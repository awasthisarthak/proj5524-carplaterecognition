...
def detectPlatesInScene(imgOriginalScene):
    ...
    if Main.showSteps == True:
        cv2.imshow("Img_Original", imgOriginalScene)

    imgGrayscaleScene, imgThreshScene = Preprocess.preprocess(imgOriginalScene)

    if Main.showSteps == True:
        cv2.imshow("Img_Grayscale", imgGrayscaleScene)
        cv2.imshow("Img_Thresholded", imgThreshScene)
    ...
    if Main.showSteps == True:
        print("\n")
        cv2.imshow("Contours_reds", imgContours)
...
def findPossibleCharsInScene(imgThresh):
    ...
    if Main.showSteps == True:
        ...
        cv2.imshow("Contours_reds2", imgContours)
